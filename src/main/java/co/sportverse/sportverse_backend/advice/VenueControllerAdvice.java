...
import co.sportverse.sportverse_backend.controller.VenueController;
...
@RestControllerAdvice(assignableTypes = VenueController.class)
public class VenueControllerAdvice {

    private static final Logger logger = LoggerFactory.getLogger(VenueControllerAdvice.class);

    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<ApiResponse> handleIllegalArgument(IllegalArgumentException e) {
        logger.warn("Venue API - invalid request: {}", e.getMessage());
        return ResponseEntity.badRequest().body(new ApiResponse(false, e.getMessage()));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponse> handleUnexpected(Exception e) {
        logger.error("Venue API - unexpected error", e);
